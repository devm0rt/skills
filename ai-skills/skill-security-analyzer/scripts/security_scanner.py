...
class EnhancedSecurityScanner:
    ...
    def _check_indirect_execution(self, content: str, file_path: Path):
        ...
        patterns = [
            ...
            (
                ...
                self.CRITICAL,
                "getattr accessing dangerous function names",
            ),
            # String concatenation to build function names
            (
                r"getattr\s*\([^,]+,\s*[^)]*\+[^)]*\)",
                self.CRITICAL,
                "Dynamic function name via string concatenation",
            ),
            ...
            (
                r'\{[^}]*[\'"](?:exec|eval|system)[\'"][^}]*\}\[',
                self.CRITICAL,
                "Dictionary-based function call obfuscation",
            ),
            # Lambda with dangerous functions
            (
                r"lambda[^:]*:\s*(?:exec|eval|__import__|getattr)",
                self.HIGH,
                "Lambda wrapping dangerous operations",
            ),
    ...
    def _check_advanced_obfuscation(self, content: str, file_path: Path):
        ...
        patterns = [
            ...
            # ROT13/Caesar cipher
            (r'codecs\.decode\([^,]+,\s*[\'"]rot', self.HIGH, "ROT cipher encoding"),
            # XOR encoding
            (r"chr\s*\(\s*ord\([^)]+\)\s*\^", self.HIGH, "XOR encoding pattern"),
            # Hex to bytes
            (
            ...
            (
                r"ast\.parse\([^)]+\).*?ast\.\w+\s*=",
                self.HIGH,
                "AST manipulation (code rewriting)",
            ),
            # Deserialization
            (
    ...
    def _check_time_bombs(self, content: str, file_path: Path):
        ...
        for pattern in time_patterns:
            for match in re.finditer(pattern, content):
                ...
                if re.search(
                    r"os\.system|subprocess|exec|eval|requests\.(post|get)|urllib|socket",
                    context,
                    re.IGNORECASE,
                ):
                    line_num = content[: match.start()].count("\n") + 1
    ...
    def _check_environment_manipulation(self, content: str, file_path: Path):
        ...
        dangerous_env_patterns = [
            ...
            (r'os\.environ\[[\'"]PATH[\'"]', "PATH manipulation (command hijacking)"),
            (
                r'os\.environ\[[\'"]PYTHONPATH[\'"]',
                "PYTHONPATH manipulation (module hijacking)",
            ),
            (r"os\.putenv\(", "Direct environment modification via putenv"),
        ]
    ...
    def _scan_reference_file(self, file_path: Path):
        ...
        try:
            ...
            html_comment_code = re.search(
                r"<!--.*?(?:exec|eval|os\.system|import os).*?-->",
                content,
            ...
            if file_path.suffix == ".md":
                ...
                for block in code_blocks:
                    if re.search(
                        r"exec\(|eval\(|os\.system\(|subprocess\..*shell\s*=\s*True",
                        block,
                    ):
                        self.findings.append(
                            {
    ...
    def _cross_file_analysis(self):
        ...
        for script_file in self.skill_path.rglob("*.py"):
            try:
                ...
                if re.search(
                    r"input\(|sys\.argv|os\.environ|request\.(args|form|json)", content
                ):
    ...
    def _generate_report(self) -> Dict:
        ...
        severity_counts = {self.CRITICAL: 0, self.HIGH: 0, self.MEDIUM: 0, self.LOW: 0}
...
def main():
    ...
    parser.add_argument(
        "--recursive", "-r", action="store_true", help="Scan all skills in directory"
    )
    ...
    output_data = (
        {"scans": all_reports}
        if args.recursive
        else (all_reports[0] if all_reports else {})
    )
